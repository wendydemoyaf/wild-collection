...
export async function GET(request: NextRequest) {
  ...
  try {
    const [currency, campaigns, today, week, month] = await Promise.all([getMetaAccountCurrency(), getAdvertisingData(), getAdvertisingData("today"), getAdvertisingData("this_week_mon_today"), getAdvertisingData("this_month")]);
    ...
    return json({ currency, campaigns: withPeriods, summary: { spend_today: total(today, "spend"), spend_week: total(week, "spend"), spend_month: total(month, "spend"), conversations, cost_per_conversation: conversations > 0 ? spend / conversations : null }, updated_at: new Date().toISOString() });
  } catch (error) {
