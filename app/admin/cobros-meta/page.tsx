...
export default function CobrosMetaPage() {
  ...
  return <main className={styles.dashboard}>
    ...
    <div className={styles.inner}>
      ...
      <section className={styles.section}><div className={styles.sectionHeading}><div><p className={styles.eyebrow}>Bloque 1</p><h2>Consumo publicitario</h2></div><p>Dinero gastado en anuncios</p></div><div className={styles.metrics}><article><small>Consumo hoy</small><strong>{money(consumption.today)}</strong></article><article><small>Consumo semana</small><strong>{money(consumption.week)}</strong></article><article><small>Consumo mes</small><strong>{money(consumption.month)}</strong></article></div><div className={styles.tableWrap}><table><thead><tr><th>Fecha</th><th>Consumo</th><th>Número de campañas activas</th></tr></thead><tbody>{consumption.daily.map((row) => <tr key={row.date}><td>{date(row.date)}</td><td>{money(row.spend)}</td><td>{row.active_campaigns}</td></tr>)}</tbody></table></div><p className={styles.note}>Las campañas se cuentan cuando Meta registra actividad publicitaria para esa fecha.</p></section>
      ...
      <section className={styles.section}><div className={styles.sectionHeading}><div><p className={styles.eyebrow}>Bloque 3</p><h2>Conciliación</h2></div><p>Consumo vs. cobros</p></div><div className={styles.reconciliation}><article><small>Consumo acumulado</small><strong>{money(reconciliation.consumption_accumulated)}</strong></article><article><small>Cobros acumulados</small><strong>{money(reconciliation.charges_accumulated)}</strong></article><article><small>Diferencia</small><strong>{money(reconciliation.difference)}</strong></article><div className={styles.pending}><b>⚠ Existe una diferencia pendiente por conciliar.</b><p>No es posible determinarla hasta contar con cobros reales. {consumption.charges_message}</p></div></div></section>
    </div>
  </main>;
}
