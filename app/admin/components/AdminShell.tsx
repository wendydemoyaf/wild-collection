...
export default function AdminShell({ children }: { children: ReactNode }) {
  ...
  function refreshSystem() {
    window.dispatchEvent(new Event("wild-admin-refresh"));
  }
  ...
  return (
    <div className={styles.shell}>
      ...
      <button type="button" className={styles.systemRefresh} onClick={refreshSystem}>Actualizar sistema</button>
      <aside className={`${styles.sidebar} ${isOpen ? styles.sidebarOpen : ""}`}>
        ...
        <nav aria-label="Navegación principal" className={styles.navigation}>
          {navigationGroups.map((group) => <section className={styles.group} key={group.title}>
            <h2>{group.title}</h2>
