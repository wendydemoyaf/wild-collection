...
function commit(value: string) {
  return value === "No disponible" ? value : value.slice(0, 7);
}
...
export default function ConfiguracionPage() {
  ...
  return <AdminShell><main className={styles.dashboard}>
    <header className={styles.header}><div><p className={styles.eyebrow}>Wild Ads Control</p><h1>Configuración e<br /><em>integraciones.</em></h1><p>Consulta el estado técnico actual del sistema. Esta sección es solo de lectura.</p></div></header>
    ...
    {data && <>
      ...
      <section className={styles.system}><div className={styles.sectionTitle}><p className={styles.eyebrow}>Referencia técnica</p><h2>Información del sistema</h2></div><dl><div><dt>Versión del panel</dt><dd>Wild Ads Control v1.0.0</dd></div><div><dt>Entorno</dt><dd>{data.system.environment}</dd></div><div><dt>Último despliegue</dt><dd>{date(data.system.last_deployment === "No disponible" ? null : data.system.last_deployment)}</dd></div><div><dt>Último commit</dt><dd>{commit(data.system.last_commit)}</dd></div></dl><small>Actualizado: {date(data.updated_at)}</small></section>
    </>}
  </main></AdminShell>;
}
